...
namespace LoraKeysManagerFacade
{
    ...
    public class LoRaADRFunction
    {
        ...
        public async Task<LoRaADRResult> HandleADRRequest(string devEUI, LoRaADRRequest request)
        {
            ...
            {
                await this.adrManager.Reset(devEUI);
            ...
            {
                return await this.adrManager.CalculateADRResultAndAddEntry(devEUI, request.GatewayId, request.FCntUp, request.FCntDown, request.RequiredSnr, request.DataRate, request.MinTxPowerIndex, newEntry);
            }
            else
            {
                await this.adrManager.StoreADREntry(newEntry);
                return await this.adrManager.GetLastResult(devEUI);
            }
        }
    }
}
